...
2. `backend/trellis/core.py` loads the node and constructs context from its path, position, ancestors, progress, and up to 12 prior interactions in that exact scope. A thread receives its saved starting context and its own history. Other nodes and other threads are excluded. `ai.resolve_question` resolves short follow-ups against the active topic and recent relevant conversation, returning one standalone question and a concise public search query. The parent topic remains background in threads. Generation and evaluation receive the same resolved question and context.
