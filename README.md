...
3. **Curriculum:** review the graph or list and expand **Original request** to compare it with your input. Goal-based creation preserves explicitly supplied phases and topics, and can infer useful subtopics. Edit titles/descriptions and parent relationships, add nodes, and reorder them. Nodes with learning history cannot be deleted. A journey retains at least one node.
...
Goal-based curriculum creation plans from the entire request before looking up evidence for each root topic. Recognized Markdown headings and topic lists retain their titles, order, and parent relationships; broader goals can gain useful subtopics. Curricula allow at most 40 topics, including parents and children. Larger requests or outputs receive an explicit error rather than silently losing topics.

Every generated topic needs source references and a separate review of support and coverage against the full original request. Acceptance requires a completeness assessment of at least 90%, no reported missing topics, preserved hierarchy, and the existing grounding checks. A negative goal review allows one research-and-revision attempt with up to three focused evidence queries; the revised curriculum must pass the same checks. **Existing curriculum** imports preserve the supplied outline through one generation and fidelity review, without web retrieval or inferred additions.

The original input, curriculum, source excerpts, assessment, model, and timestamp remain inspectable after learner edits. Older saved assessments with low completeness or reported omissions show a coverage warning without rewriting their original result. Older journeys without recorded generation evidence are labelled accordingly. Model assessments remain quality signals, not guarantees of complete coverage or factual correctness.
